...
# Parameters from Mycoplasma pneumoniae SIRS model (Pathogen 2)
beta2 = 1.671843
mu2 = 1.051216
xi2 = 0.057486

# Other fixed parameters in the model 0.371396895216788
theta1 =  0.360863
upsilon = xi1
...
try:
    ...
    mp_start_row = df[(df['Year'] == 2023) & (df['Week'] == 52)].iloc[0]
    I2_0_fitted = mp_start_row['Weekly_Mp_Positive_Rate'] / 1000
    print(f"MP I0 (2023-W52) read from data: {I2_0_fitted:.6f}")
...
output_filename = 'heatmap_14comp_abs_ratio_data_step001.csv'
...
if np.any(np.isfinite(plot_data_log10)):
    ...
    # 1. Plot the (+) part
    ax.scatter(mp_lambda, mp_theta2, 
               marker='+', 
               color='yellow', 
               s=marker_size,
    ...
    # Add the text label
    ax.text(mp_lambda + 0.015, mp_theta2, 'MP', 
            ha='left', 
            va='center', 
            color='black', 
            fontsize=24,
...
plt.tight_layout()
plt.savefig("heatmap_14comp_abs_ratio_inf_mp_step001.png")
plt.show()

print("\nScript finished. Heatmap plot saved as 'heatmap_14comp_abs_ratio_inf_mp_step001.png'")