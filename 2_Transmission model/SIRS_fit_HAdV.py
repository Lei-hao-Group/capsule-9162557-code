...
# Filter Time Period (2023-W52 to 2024-W51)
time_filter = ((df['Year'] == 2023) & (df['Week'] >= 52)) | ((df['Year'] == 2024) & (df['Week'] <= 51))
df_filtered = df.loc[time_filter].copy()
df_filtered.sort_values(by=['Year', 'Week'], inplace=True)

observed_data = df_filtered['Weekly_HADV_Positive_Rate'].values / 100 / 10
t_values = np.arange(len(observed_data))
print(f"Data for HAdV filtered from {df_filtered['YearWeek'].iloc[0]} to {df_filtered['YearWeek'].iloc[-1]}. Total weeks: {len(observed_data)}")
...
# Define Parameter Bounds (Keep HAdV Original Beta < 5.0)
PRIOR_BOUNDS = {
    'beta': (0.0, 5.0), 
    'mu':   (0.0, 5.0),
    'xi':   (0.0, 1.0)
...
print("\nStarting emcee sampling for HAdV SIRS model...")
...
print("\nCalculating 95% credible interval for HAdV fit...")
...
ax1.plot(t_values, observed_data, 'o', color='black', alpha=0.7, label='Observed HAdV Data', markersize=6)
ax1.plot(t_values, median_prediction, color='red', lw=3, label='SIRS Model Fit (Median)')

# [Modification]: Complete X-axis label, corresponding to HAdV from 2023-W52 to 2024-W51
ax1.set_xlabel('Time (Weeks from 2023-W52 to 2024-W51)')
...
output_file = 'HADV_sirs_3panel_results.png'
