...
observed_data = df_filtered['Weekly_Mp_Positive_Rate'].values / 100 / 10
t_values = np.arange(len(observed_data))
print(f"Data for Mp filtered from {df_filtered['YearWeek'].iloc[0]} to {df_filtered['YearWeek'].iloc[-1]}. Total weeks: {len(observed_data)}")
...
# Define Parameter Bounds (Keep Mp Original Beta < 5.0)
PRIOR_BOUNDS = {
    'beta': (0.0, 5.0), 
    'mu':   (0.0, 5.0),
    'xi':   (0.0, 1.0)
...
print("\nStarting emcee sampling for Mp SIRS model...")
...
print("\nCalculating 95% credible interval for Mp fit...")
...
ax1.plot(t_values, observed_data, 'o', color='black', alpha=0.7, label='Observed MP Data', markersize=6)
ax1.plot(t_values, median_prediction, color='red', lw=3, label='SIRS Model Fit (Median)')

# [Modification]: Complete X-axis label, corresponding to Mp from 2023-W52 to 2024-W51
...
output_file = 'MP_sirs_3panel_results.png'
