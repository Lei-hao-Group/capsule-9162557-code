...
# Specific slicing logic (Week 44, 2023 + 52 weeks)
try:
    start_index = df[(df['Year'] == 2023) & (df['Week'] == 44)].index[0]
    ...
    # Get HMPV data
    observed_data = df_filtered['Weekly_HMPV_Positive_Rate'].values / 100 / 10
    t_values = np.arange(len(observed_data))
    print(f"Data for HMPV filtered from {df_filtered['YearWeek'].iloc[0]} to {df_filtered['YearWeek'].iloc[-1]}. Total weeks: {len(observed_data)}")

except IndexError:
    print("Error: Could not find the start week (Week 44, 2023) in the data file.")
    exit()
...
# Define parameter bounds (Keep HMPV Beta < 10.0)
PRIOR_BOUNDS = {
    'beta': (0.0, 10.0), 
    'mu':   (0.0, 5.0),
    'xi':   (0.0, 1.0)
...
print("\nStarting emcee sampling for HMPV SIRS model...")
...
print("\nCalculating 95% credible interval for HMPV fit...")
...
ax1.plot(t_values, observed_data, 'o', color='black', alpha=0.7, label='Observed HMPV Data', markersize=6)
ax1.plot(t_values, median_prediction, color='red', lw=3, label='SIRS Model Fit (Median)')

# [Modification]: Complete X-axis label, corresponding to 52 weeks (2023-W44 to 2024-W43)
ax1.set_xlabel('Time (Weeks from 2023-W44 to 2024-W43)')
...
output_file = 'HMPV_sirs_3panel_results.png'
