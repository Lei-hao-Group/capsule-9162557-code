...
# Parameters from RSV SIRS model (Pathogen 2)
beta2 = 3.570330
mu2 = 1.241515   
xi2 = 0.009299   
...
try:
    ...
    # Pathogen 2 (RSV)
    rsv_start_row = df[(df['Year'] == 2023) & (df['Week'] == 48)].iloc[0]
    I2_0_fitted = rsv_start_row['Weekly_HRSV_Positive_Rate'] / 1000 # /100 /10 is /1000
    print(f"RSV I0 (2023-W48) read from data: {I2_0_fitted:.6f}")
...
output_filename = 'heatmap_14comp_abs_ratio_inf_rsv_step001.csv'
...
if np.any(np.isfinite(plot_data_log10)):
    ...
    # --- Plot RSV data point as "米" shape ---
    marker_lambda = 0.874  
    marker_theta2 = 0.885976  
    ...
    # Add the text label
    ax.text(marker_lambda + 0.015, marker_theta2, 'RSV',  
            ha='left', 
            va='center', 
            color='black', 
            fontsize=24,
...
plt.tight_layout()
plt.savefig("heatmap_14comp_abs_ratio_inf_rsv_step001.png")
plt.show()

print("\nScript finished. Heatmap plot saved as 'heatmap_14comp_abs_ratio_inf_rsv_step001.png'") 